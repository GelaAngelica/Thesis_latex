...
%The solution-diffussion model are based on the assumption that the driving force of the solvent through the RO-membrane is the gradient of its chemical potential. It is assumed that the solvent is pure water and that the solute is salt. The pressure within the membrane through which transportation takes place is uniform and the chemical potential gradient can be expressed in Fick's law, where the flux of a component i can be described as:

%\begin{equation}
%J_{i}=-D\frac{dC_{i}}{dz} 
%\end{equation}
%where $D_{i}$ is the component diffusivity and $C_{i}$ is the component concentration within the membrane module. 
